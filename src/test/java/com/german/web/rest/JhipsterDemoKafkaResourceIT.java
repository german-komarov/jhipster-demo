...
import com.german.IntegrationTest;
import com.german.config.EmbeddedKafka;
import com.german.config.KafkaSseConsumer;
import com.german.config.KafkaSseProducer;
...
class JhipsterDemoKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/jhipster-demo-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/jhipster-demo-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/jhipster-demo-kafka/unregister"));
                return;
            }
        }
